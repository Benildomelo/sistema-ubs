...
class PatientDashboard {
    ...
    loadUserData() {
        ...
        if (userNameElement && this.currentUser) {
            ...
            if (welcomeMessageElement) {
                welcomeMessageElement.textContent = 'Bem-vindo de volta';
            }
        }
    ...
    updateUserAvatar() {
        const avatarContainer = document.querySelector('.flex.items-center.gap-3 .flex.size-10');
        ...
        if (avatarContainer && this.currentUser) {
            ...
            svg.setAttribute('style', `background-color: ${avatar.color.background};`);
    ...
    setupLogoutButton() {
        ...
        logoutBtn.className = 'logout-btn flex items-center justify-center w-10 h-10 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors';
        logoutBtn.innerHTML = `
    ...
    setupNotifications() {
        const notificationBtn = document.querySelector('button .material-symbols-outlined');
    ...
    getUserAppointments() {
        try {
            ...
            if (appointments.length === 0) {
                appointments = JSON.parse(localStorage.getItem('ubs_appointments')) || [];
            }

            console.log('📋 Todas as consultas no sistema:', appointments);

            const userAppointments = appointments.filter(appt => {
                const isUserAppointment = appt.paciente && appt.paciente.cpf === this.currentUser.cpf;
                console.log('Consulta:', appt, 'Pertence ao usuário?', isUserAppointment);
                return isUserAppointment;
            });

            console.log('👤 Consultas do usuário:', userAppointments.length, userAppointments);
    ...
    checkTodayAppointments() {
        ...
        this.todayAppointments = appointments.filter(appt => {
            ...
            const isValidStatus = appt.status === 'agendada' || appt.status === 'confirmada';
    ...
    updateNotificationBadge() {
        ...
        const existingBadge = notificationBtn.parentElement.querySelector('.notification-badge');
        if (existingBadge) {
        ...
        if (this.todayAppointments.length > 0) {
            ...
            badge.className = 'notification-badge absolute top-1 right-1 size-3 bg-red-500 rounded-full border-2 border-white dark:border-gray-800';
    ...
    showTodayAppointmentsAlert() {
        ...
        if (this.todayAppointments.length > 0 && lastAlertDate !== today) {
            setTimeout(() => {
                const appointmentText = this.todayAppointments.length === 1 ?
                    'você tem 1 consulta agendada para hoje!' :
                    `você tem ${this.todayAppointments.length} consultas agendadas para hoje!`;

                if (confirm(`📅 Olá ${this.currentUser.firstName || this.currentUser.name.split(' ')[0]}! ${appointmentText} Clique em OK para ver os detalhes.`)) {
                    this.showNotificationsModal();
                }
                ...
                <div class="p-4 overflow-y-auto">
                    ${this.todayAppointments.length > 0 ? `
                        <div class="mb-4">
                            ...
                            ${this.todayAppointments.map(appt => `
                    ...
                    ${this.getUpcomingAppointments().length > 0 ? `
                        <div class="mt-4">
                            ...
                            ${this.getUpcomingAppointments().map(appt => `
                                <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3 mb-2">
                                    <p class="font-medium text-blue-800 dark:text-blue-300">${appt.especialidade}</p>
                                    <p class="text-sm text-blue-600 dark:text-blue-400">${this.formatDate(appt.data)} às ${appt.horario}</p>
                                    <p class="text-xs text-blue-500 dark:text-blue-500 mt-1">${appt.unidade}</p>
                                </div>
                            `).join('')}
                        </div>
                    ` : ''}
    ...
    loadMyAppointments() {
        ...
        this.renderAppointmentList('my-appointments-list', futureAppointments, 'agendadas');
    }
    ...
    loadAppointmentHistory() {
        ...
        const historico = appointments.filter(appt => {
            ...
            const isNotActive = appt.status !== 'agendada' && appt.status !== 'confirmada';
        ...
        this.renderAppointmentList('appointment-history-list', historico, 'historico');
    }

    renderAppointmentList(containerId, appointments, tipo) {
        console.log(`🎨 RENDERIZANDO ${tipo} no container: ${containerId}`, appointments);
        ...
        if (appointments.length === 0) {
            container.innerHTML = `
                <div class="text-center py-8 text-gray-500">
                    <span class="material-symbols-outlined text-4xl mb-2">${tipo === 'agendadas' ? 'calendar_today' : 'history'}</span>
                    <p class="mb-2 font-medium">${tipo === 'agendadas' ? 'Nenhuma consulta agendada' : 'Nenhuma consulta no histórico'}</p>
                    ${tipo === 'agendadas' ? `
        ...
        container.innerHTML = appointments.map(appt => {
            const statusConfig = {
                'agendada': { text: 'Agendada', color: 'text-yellow-600', bgColor: 'bg-yellow-100' },
                'confirmada': { text: 'Confirmada', color: 'text-green-600', bgColor: 'bg-green-100' },
                'concluída': { text: 'Concluída', color: 'text-blue-600', bgColor: 'bg-blue-100' },
                'cancelada': { text: 'Cancelada', color: 'text-red-600', bgColor: 'bg-red-100' }
            };

            const status = statusConfig[appt.status] || statusConfig.agendada;

            const botoes = tipo === 'agendadas' ? `
            ...
            return `
                <div class="flex items-center gap-4 bg-white dark:bg-gray-800 p-4 rounded-lg shadow mb-2 hover:shadow-md transition-shadow">
                    ...
                    <div class="flex-1">
                        <p class="font-bold text-gray-900 dark:text-white">${appt.especialidade}</p>
                        <p class="text-gray-600 dark:text-gray-300">${this.formatDate(appt.data)} às ${appt.horario}</p>
                        <p class="text-gray-600 dark:text-gray-300">${appt.unidade}</p>
                        <p class="text-gray-600 dark:text-gray-300">Dr(a). ${appt.profissional.nome}</p>
                        <span class="inline-block px-2 py-1 text-xs font-medium rounded-full ${status.color} ${status.bgColor}">
                            ${status.text}
                        </span>
                    </div>
                    ${botoes}
        ...
        if (confirm('Tem certeza que deseja cancelar esta consulta?')) {
            try {
                ...
                if (appointmentIndex !== -1) {
                    ...
                    localStorage.setItem('consultasAgendadas', JSON.stringify(appointments));
